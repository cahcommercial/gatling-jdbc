...
case class JdbcInsertAction(requestName: Expression[String],
  ...
  override def execute(session: Session): Unit = {
    ...
    val validatedTableName: Validation[String] = tableName.apply(session)
    val validatedValues = values.apply(session)

    //TODO: fix how to get String from Validation[String]. .toString returns "Success(String..." so substring starting at 8 works
    var justTableName = validatedTableName.toString
    justTableName = justTableName.substring(8, justTableName.indexOf(" "))

    val result: Validation[Future[Unit]] = for {
      tableValue <- validatedTableName
      valuesValue <- validatedValues

      sql <- Success(s"SET IDENTITY_INSERT $justTableName ON; INSERT INTO $tableValue VALUES ( $valuesValue ); SET IDENTITY_INSERT $justTableName OFF;")
    } yield {
      Future {
